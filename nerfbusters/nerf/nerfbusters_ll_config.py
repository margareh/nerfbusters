...
nerfbusters_config = MethodSpecification(
    TrainerConfig(
        method_name="nerfbusters-ll",
        project_name="nerfbusters-ll-project",
        ...
        pipeline=NerfbustersPipelineConfig(
            datamanager=VanillaDataManagerConfig(
                dataparser=LunarLabDataParserConfig(eval_mode="eval-frame-index"),
                train_num_rays_per_batch=4096,
                eval_num_rays_per_batch=4096,
            ),
            model=NerfactoLLModelConfig(eval_num_rays_per_chunk=1 << 15, predict_normals=True, depth_method="median"),
        ),
        optimizers={
            "proposal_networks": {
